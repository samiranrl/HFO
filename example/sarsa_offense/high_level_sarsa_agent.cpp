...
void offenseAgent(int basePort, int port, int numTMates, int numEpi, double learnR,
  ...
  int action = -1;
  double reward;
  hfo.connectToServer(hfo::HIGH_LEVEL_FEATURE_SET,"../../bin/teams/base/config/formations-dt",basePort,"localhost","base_left",false,"");
...
int main(int argc, char **argv) {
  ...
  for (int agent = 0; agent < numAgents; agent++) {
    agentThreads[agent] = std::thread(offenseAgent, basePort, basePort + agent,
